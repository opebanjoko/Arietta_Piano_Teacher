...
/**
 * Scores set-valued events against set-valued ground truth (Phase 6 poly gate).
 *
 * events: [{ midis: [..], timestamp }], truth: [{ midis: [..], atMs }]
 * A truth chord is detected iff one event within the window carries exactly
 * its midi set. An unmatched event is FALSE only if it contains a pitch that
 * was not sounding nearby (a phantom); a strict subset of a played chord is
 * a miss, already punished by detectionRate, not a false positive - the
 * false bound (SR-AUD-07) is about hearing notes that were not played.
 */
export function scoreSets(events, truth, { durationMs, matchWindowMs = 500 } = {}) {
  const key = (midis) => midis.join(',');
  const matched = new Set();
  const latencies = [];
  let falseEvents = 0;
  ...
  const sorted = [...events].sort((a, b) => a.timestamp - b.timestamp);
  for (const e of sorted) {
    const ek = key(e.midis);
    let best = -1;
    let bestDist = Infinity;
    for (let i = 0; i < truth.length; i++) {
      if (matched.has(i) || key(truth[i].midis) !== ek) continue;
      const dist = Math.abs(e.timestamp - truth[i].atMs);
      if (dist <= matchWindowMs && dist < bestDist) {
        bestDist = dist;
        best = i;
    ...
    if (best >= 0) {
      ...
      const subsetOfPlayed = truth.some(
        (t) => Math.abs(e.timestamp - t.atMs) <= matchWindowMs && e.midis.every((m) => t.midis.includes(m))
      );
      if (!subsetOfPlayed) falseEvents++;
    }
  }
